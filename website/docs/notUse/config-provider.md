...
```html
<template>
  <ly-config-provider :locale="locale">
    <app />
  </ly-config-provider>
</template>
...
<script>
  ...
  import { ElConfigProvider } from 'ly-ui'
...
```html
<div>
  <ly-config-provider :locale="locale1">
  ...
  <ly-button @click="toggle" style="margin-left: 8px; vertical-align: middle;">
    切换语言
  </ly-button>
</div>
<script>
  import { ref } from 'vue'
  // import { ConfigProvider } from 'ly-ui'
  export default {
    setup() {
      const locale1 = ref({
        name: 'zh-cn',
