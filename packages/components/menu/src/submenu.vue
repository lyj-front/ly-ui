...
export default defineComponent({
  ...
  setup(props: ISubMenuProps) {
    ...
    const isFirstLevel = computed(() => {
      ...
      while (parent && parent.type.name !== 'LyMenu') {
        if (['LySubMenu', 'LyMenuItemGroup'].includes(parent.type.name)) {
          isFirstLevel = false
          break
        } else {
    ...
    const handleMouseleave = (deepDispatch = false) => {
      ...
      if (appendToBody.value && deepDispatch) {
        if (instance.parent.type.name === 'LySubMenu') {
  ...
  render() {
    ...
    // temporaryly mark LyPopper as any due to type inconsistency.
