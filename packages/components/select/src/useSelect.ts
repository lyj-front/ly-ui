...
import type { LyFormContext, LyFormItemContext } from '@element-plus/tokens'
...
export const useSelect = (props, states: States, ctx) => {
  ...
  const elFormItem = inject(elFormItemKey, {} as LyFormItemContext)
  ...
  const iconClass = computed(() =>
    ...
    //   ? ''
    //   : states.visible
    //     ? 'caret-top is-reverse is-black'
    //     : 'caret-top is-black'
    states.visible ? 'caret-top is-reverse is-black' : 'caret-top is-black'
  )
  ...
  const emptyText = computed(() => {
    if (props.loading) {
      ...
      if (props.remote && states.options.size === 0)
  ...
  watch(
    ...
    (val, oldVal) => {
      ...
      if (props.showAll) {
        states.allSelected = optionsAllSelected.value
      }
      setSelected()
  ...
  const resetInputHeight = () => {
    ...
    nextTick(() => {
      ...
      const sizeInMap = states.initialInputHeight || 30
      input.style.height =
        states.selected.length === 0
          ? sizeInMap + 'px'
          : Math.max(
  ...
  const handleQueryChange = (val) => {
    ...
    if (props.showAll) {
      setTimeout(() => {
        if (states.filteredOptionsCount === 0) {
          states.allSelected = false
          return
        }
        states.allSelected = optionsAllSelected.value
      }, debounce.value)
  ...
  const managePlaceholder = () => {
    if (states.currentPlaceholder !== '') {
      ...
      states.currentPlaceholder =
        props.remote && input.value.value ? '' : states.cachedPlaceHolder
    }
  }
  ...
  const getOption = (value) => {
    ...
    for (let i = states.cachedOptions.size - 1; i >= 0; i--) {
      ...
      if (isEqualValue) {
        option = {
          value,
          currentItem: deepClone(cachedOption.item || {}),
  ...
  //是否选择了全部
  const optionsAllSelected = computed(() => {
    const tempArr = optionsArray.value.filter(
      (option) => option.visible && !option.itemSelected
    )
    return tempArr.length === 0
  })

  //取消或者全选
  const handleOptionSelectAll = (value, byClick) => {
    if (props.multiple) {
      ctx.emit(UPDATE_MODEL_EVENT, value)
      emitChange(value)
    }
  ...
  const selectAllChange = () => {
    const unSelectList = [], //未选中的项
      allList = [], //未搜索时全部的选项
      selectList = [], //搜索前选中的项
      searchAllList = [] //搜索时全部的选项
    for (let i = 0; i < optionsArray.value.length; i++) {
      const item = optionsArray.value[i]
      allList.push(item.value)
      if (item.visible) {
        searchAllList.push(item.value)
      }
      if (!item.visible && item.itemSelected) {
        selectList.push(item.value)
      }
      if (item.visible && !item.itemSelected) {
        unSelectList.push(item.value)
      }
    }
    //判断有无进行值搜索
    if (states.options.size === states.filteredOptionsCount) {
      //未进行值搜索
      if (unSelectList.length !== 0) {
        const value = (props.modelValue || []).slice()
        allList.forEach(item=>{
          const optionIndex = getValueIndex(value, item)
          if (optionIndex === -1) {
            value.push(item)
          }
        })
        handleOptionSelectAll(value, true)
      } else {
        const value = (props.modelValue || []).slice()
        allList.forEach(item=>{
          const optionIndex = getValueIndex(value, item)
      ...
      if (unSelectList.length !== 0) {
        //未全部选中，进行全部勾选
        handleOptionSelectAll(searchAllList.concat(...selectList), true)
      } else {
        //已全部选中，取消勾选
        handleOptionSelectAll(selectList, true)
      }
    }
  }
