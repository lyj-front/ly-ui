...
export default class TreeStore {
  ...
  setData(newVal: TreeData): void {
    ...
    if (instanceChanged) {
      ...
      this._initDefaultCheckedNodes()
      // if (this.defaultCheckedNodeKeys) {
      //   this.setDefaultCheckedNodeKeys(this.defaultCheckedNodeKeys)
      // }
    } else {
  ...
  _initDefaultCheckedNodes(): void {
    ...
    if (defaultCheckedKeys.indexOf('all') !== -1) {
      // 默认所中所有
      for (const checkedKey in this.nodesMap) {
        const node = this.nodesMap[checkedKey]
        node.setChecked(true, false, true, false, false)
      }
    } else {
