<template>
  ...
  >
    ...
    <!-- :class="[
          node.nextSibling
            ? 'el-tree-node__shu_xian'
            : 'el-tree-node__ban_shu_xian',
        ]" -->
    <div class="el-tree-node__content">
      <span
        v-for="key of node.level - 1"
        :key="key"
        class="el-tree-node__shu_xian"
      ></span>
      <!-- expanded: !node.isLeaf && expanded, 做为展开缩起的动画，暂时被抛弃了 -->
      <span
        v-if="!node.isLeaf"
        :class="[
          {
          ...
          tree.props.iconClass
            ...
            ? 'el-icon-jianhaoshouqi'
            : 'el-icon-jiahaozhankai',
        ]"
      ...
      <span
        ...
        :class="[
          !node.nextSibling
            ? `el-tree-node__ban_xian${showCheckbox ? '_checkbox' : ''}`
            : `el-tree-node__zhong_xian${showCheckbox ? '_checkbox' : ''}`,
        ]"
      ></span>
      <ly-checkbox
        v-if="showCheckbox"
...
import LyCollapseTransition from '@element-plus/components/collapse-transition'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const handleCheckChange = (value, ev) => {
      // const deep = tree.props.checkStrictly // 原来的
