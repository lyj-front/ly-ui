  ...
  >
    <template #trigger>
      ...
      >
        ...
        <ly-tooltip
          placement="top-start"
          :popper-class="`ly-top-tip `"
          :fallback-placements="['bottom-start', 'top-start']"
          effect="light"
          :disabled="!(multiple && collapseTags && presentTags.length > 0)"
        >
          <template #content>
            <div
              :style="{ width: inputWidth - 2 + 'px', 'overflow-x': 'hidden' }"
              class="top-dark"
            >
              <ly-tag
                v-for="tag in presentTagsAll"
                :key="tag.key"
                type="info"
                :size="tagSize"
                :hit="tag.hitState"
...
import LyCascaderPanel, {
  CommonProps,
} from '@element-plus/components/cascader-panel'
import LyInput from '@element-plus/components/input'
import LyPopper from '@element-plus/components/popper'
...
export default defineComponent({
  ...
  directives: {
    ...
    showToast: {
      mounted(el) {
        ...
        // tag有省略的话就移上去显示全称
        if (el.scrollWidth > el.clientWidth) el.title = el.innerText
      },
    },
  },
  ...
  setup(props, { emit }) {
    ...
    const presentTagsAll: Ref<Tag[]> = ref([])
    const suggestions: Ref<CascaderNode[]> = ref([])
    const inputWidth = ref(222)
    ...
    const checkedValue = computed<CascaderValue>({
      get() {
        panel.value?.syncCheckedValue(true, false)
    ...
    const calculatePresentTags = () => {
      ...
      if (nodes.length) {
        ...
        tagsAll.push(genTag(first))

        if (restCount) {
          rest.forEach((node) => tagsAll.push(genTag(node)))
    ...
    const updateStyle = () => {
      ...
      // console.log(inputInner.clientWidth, 'inputInner')
    }
    ...
    watch(presentTags, () => {
      ...
      let el = cascader?.value
      if (
        multiple.value &&
        el &&
        props.collapseTags &&
        presentTags.value.length > 1
      ) {
