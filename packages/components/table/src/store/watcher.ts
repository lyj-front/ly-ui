// @ts-nocheck
import { getCurrentInstance, ref, toRefs, unref,shallowRef, watch } from 'vue'
...
function useWatcher<T>() {
  ...
  // 检查 rowKey 是否存在
  const assertRowKey = () => {
    if (!rowKey.value) throw new Error('[LyTable] prop row-key is required')
  }
  ...
  const _toggleAllSelection = () => {
    // 根据 `selectOnIndeterminate` 确定全选或全取消
    ...
    const treeProps = {
      ...
      checkStrictly: false, // 禁用 checkStrictly 以便全选
    }
  
    // 对所有行进行选择或取消选择
    data.value.forEach((row, index) => {
      const rowIndex = index + childrenCount
      const statusChanged = toggleRowStatus(
        selection.value,
        row,
        value,
        treeProps,
    ...
    // 如果有变动，则触发事件
    if (selectionChanged) {
      const updatedSelection = selection.value ? [...selection.value] : []
      instance.emit('selection-change', updatedSelection)
    }
  
    // 触发 select-all 事件
    instance.emit('select-all', selection.value ? [...selection.value] : [])
  }
