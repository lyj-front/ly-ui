...
export const getColumnByKey = function <T>(
  ...
  if (!column)
    throwError('Table', `No column matching with column-key: ${columnKey}`)
  return column
}
...
export function toggleRowStatus<T>(
  ...
  rowIndex: number = 0 // 默认值
): boolean {
  let changed = false;

  // 工具函数：添加或移除行状态
  const toggleStatus = (shouldAdd: boolean) => {
    if (shouldAdd) {
      statusArr.push(row);
    } else {
  ...
  // 工具函数：计算所有子节点数量
  const getChildrenCount = (row: T): number => {
    const childrenKey = tableTreeProps?.children;
    const children = childrenKey && row[childrenKey];
    if (Array.isArray(children)) {
      return children.reduce((count, child) => count + 1 + getChildrenCount(child), 0);
    }
    return 0;
  };

  // 判断行是否可选
  const isRowSelectable = selectable ? selectable(row, rowIndex) : true;

  // 主逻辑：根据 `newVal` 和 `selectable` 决定状态切换
  if (isRowSelectable) {
    const included = statusArr.includes(row);
    const shouldAdd = isBoolean(newVal) ? newVal : !included;
    if (shouldAdd !== included) {
      toggleStatus(shouldAdd);
    }
  }
