...
function useStyle<T>(
  ...
  const resizeListener = () => {
    ...
    const width = (tableWidth.value = el.clientWidth - borderLeft - borderRight); // `clientWidth` 更精准
    if (oldWidth !== width) {
      shouldUpdateLayout = true;
    }

    const height = el.clientHeight;
    if (((props.height && props.height !== 'auto') || shouldUpdateHeight.value) && oldHeight !== height) {
      shouldUpdateLayout = true;
    }

    const tableHeader = props.tableLayout === 'fixed'
