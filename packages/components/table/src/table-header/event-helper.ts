...
import { isClient } from '@element-plus/utils/browser'
...
function useEvent<T>(props: TableHeaderProps<T>, emit) {
    ...
    if (draggingColumn.value) {
      ...
      const handleMouseUp = () => {
        ...
        setTimeout(() => {
          ...
          removeClass(columnEl, 'drag-hover')
        }, 0)
      }
  ...
  const handleMouseMove = (event: MouseEvent, column: TableColumnCtx<T>) => {
    ...
    if (!dragging.value) {
      ...
      if (rect.width > 12 && rect.right - event.pageX < 16 && !isLastTh) {
        addClass(target, 'drag-hover')
