...
function useRender<T>(
  ...
  const checkSubColumn = (children: TableColumn<T> | TableColumn<T>[]) => {
    ...
    function check(item: TableColumn<T>) {
      if (item?.type?.name === 'LyTableColumn') {
  ...
  const setColumnRenders = (column: TableColumnCtx<T>) => {
    ...
    if (column.type === 'expand') {
      ...
      if (useTableConfig.tableConfigControl && useTableConfig.tableConfigControl.lineHeight) {
        styleProps.style.lineHeight = `${useTableConfig.tableConfigControl.lineHeight - 12}px`
      }
      ...
      column.renderCell = (data) => {
        ...
        if (useTableConfig.tableConfigControl && useTableConfig.tableConfigControl.lineHeight) {
          props.style.lineHeight = `${useTableConfig.tableConfigControl.lineHeight - 12}px`
        }
