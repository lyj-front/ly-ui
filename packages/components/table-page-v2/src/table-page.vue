<template>
  ...
  <ly-container
    :class="['ly-table-page ly-table-page-v2', customClass]"
  ...
  >
    ...
    <div class="block-line-10" v-if="search"></div>
    <ly-main>
      <ly-table-v2
        v-bind="$attrs"
        :data="tableData"
...
<script>
import { ref, computed, toRefs, defineComponent, provide } from 'vue'
...
function getTableData(tableConfig, props, emit) {
  ...
  async function getInfo() {
    ...
    try {
      ...
      if (!responseMap.List) tableData.value = res.data
      else tableData.value = responseMapGetData(res.data, responseMap.List)
      if (isPage) {
        pageInfo.Total = responseMapGetData(res.data, responseMap.Total)
        getTotal()
        // pageInfo.No = responseMapGetData(res.data, responseMap.No)
      }
      return res
    } catch (error) {
      tableLoading.value = false
