...
function useWatcher<T>() {
  ...
  const isSelected = (row) => {
    const findIndex = selection.value.findIndex(item=>item[rowKey.value]===row[rowKey.value])
    return selection.value.indexOf(row) > -1 || findIndex > -1
  }
  ...
  const toggleRowSelection = (
    ...
    const changed = toggleRowStatus(selection.value, row, selected,rowKey.value)
    if (changed) {
  ...
  const _toggleAllSelection = () => {
    ...
    _data.value.forEach((row, index) => {
      const rowIndex = index + childrenCount
      if (selectable.value) {
        if (
          selectable.value.call(null, row, rowIndex) &&
          toggleRowStatus(selection.value, row, value,rowKey)
        ) {
          selectionChanged = true
        }
      } else {
        if (toggleRowStatus(selection.value, row, value,rowKey)) {
          selectionChanged = true
        }
      }
