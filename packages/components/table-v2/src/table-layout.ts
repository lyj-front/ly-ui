...
class TableLayout<T> {
  ...
  updateColumnsWidth() {
    ...
    this.scrollWidth.value = 0
    const arr = this.store.states?._columns.value
    for(let i = 0;i<arr.length;i++) {
      this.scrollWidth.value+=Number(arr[i].width||arr[i].realWidth)
    }
